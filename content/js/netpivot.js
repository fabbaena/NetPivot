...
function showdata(data) {
    ...
    attributelist.append($("<tr>")
        ...
        .append($("<th>").width("50px").html("Conv."))
    ...
    $.getJSON("../engine/get_attributes_json.php", 
        ...
        function(data) {
            ...
            for(attribute in attributes) {
                ...
                if(attributes[attribute].converted) {
                ...
                switch(attribute) {
                    case "profiles":
                        for(p in attributes.profiles.value) {
                            profile = attributes.profiles.value[p];
                            var link = {
                                    "mod": "ltm", 
                                    "og": "profile", 
                                    "on": p
                            ...
                            if(attributes[attribute].converted) {
                                $("#lineno" + profile.line).addClass("linked_line");
                            }
                            $("#conv" + profile.line)
                                .html(gotoIcon(link));
                        }
                    ...
                    case "pool":
                        ...
                        break;
                    case "members":
                        for(m in attributes.members.value) {
                            member = attributes.members.value[m];
                            var link = {
                                    "mod": "ltm", 
                                    "og": "node", 
                                    "on": m.split(":")[0]
                                };
                            $("#lineno" + member.line).addClass("linked_line");
                            $("#conv" + member.line)
                                .html(gotoIcon(link));
                        }
                        break;
